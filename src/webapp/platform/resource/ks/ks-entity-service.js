                ...
                var _ksEntityService = {
                        ...
                        switch (service.method) {
                            ...
                            case "DELETE":
                                return _ksEntityService.delete(url, data).error(function (msg,state) {
                                    _ksEntityService.stateErrorTip("删除失败",msg,state);
                                });
                                break;
                        }
                    },
                    ...
                    uploadMultiFile:function(url,datas,fileFields,success,error,outTime){
                        ...
                        for(var i=0;i<fileFields.length;i++){
                            ...
                            if(fileData){
                                var filename=name+"."+fileData.type.substring(fileData.type.indexOf("/")+1)
                                formData.append(name,fileData,filename);
                            }
                            delete datas[fileName];
                        }
                        for(var key in datas){
                            if(datas[key] || (!datas[key] &&(datas[key]===0 || datas[key]===false))){
